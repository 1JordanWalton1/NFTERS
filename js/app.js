(() => {
    ...
    class Popup {
        ...
        _openToHash() {
            ...
            if (!hash || !/^[a-zA-Z0-9_-]+$/.test(hash)) {
                this.popupLogging(`Некорректный хеш: ${hash}`);
                return;
            }
            const classInHash = `.${hash}`;
            const idInHash = `#${hash}`;
            const element = document.querySelector(classInHash) || document.querySelector(idInHash);
            if (element) {
                const buttons = document.querySelector(`[${this.options.attributeOpenButton}="${classInHash}"]`) || document.querySelector(`[${this.options.attributeOpenButton}="${idInHash}"]`);
                if (buttons) {
                    this.youTubeCode = buttons.getAttribute(this.options.youtubeAttribute) || null;
                    this.open(classInHash);
                } else this.popupLogging(`Не найдена кнопка для открытия попапа с селектором: ${classInHash}`);
            } else this.popupLogging(`Элемент с селектором ${classInHash} или ${idInHash} не найден.`);
        }
        _setHash() {
    ...
    function initSliders() {
        if (document.querySelector(".card-hero__slider")) new swiper_core_Swiper(".card-hero__slider", {
    ...
    function updateTimers() {
        const timerElements = document.querySelectorAll(".hero-slide__timer");
        if (timerElements.length === 0) {
            console.error('Элементы с классом "hero-slide__timer" не найдены.');
            return;
        }
        timerElements.forEach((timerElement => {
            let time = 12 * 3600 + 43 * 60 + 42;
            setInterval((() => {
                time--;
                const hours = Math.floor(time / 3600);
                const minutes = Math.floor(time % 3600 / 60);
                const seconds = time % 60;
                timerElement.innerHTML = `${hours.toString().padStart(2, "0")}<span>h</span> \n                 ${minutes.toString().padStart(2, "0")}<span>m</span> \n                 ${seconds.toString().padStart(2, "0")}s`;
            }), 1e3);
        }));
    }
    if (document.querySelectorAll(".hero-slide__timer").length > 0) updateTimers(); else console.warn('Элементы с классом "hero-slide__timer" не найдены. Таймеры не запущены.');
